I need to generate test data samples for my payment engine.

It's an engine that processes payments; it receives a .csv file with transaction information and should output another .csv file with the account information.

This is the description about the input:
...
This is the description about the output:

The output should be a list of client IDs (client), available amounts (available), held amounts
(held), total amounts (total), and whether the account is locked (locked). Columns are defined as
...
deposit: A deposit is a credit to the client's asset account, meaning it should increase the available and
...
Withdrawal: A withdraw is a debit to the client's asset account, meaning it should decrease the available and
total funds of the client account
...
Resolve: A resolve represents a resolution to a dispute, releasing the associated held funds. Funds that
...
Please generate around 30 pair samples (transactions + expected output), with around 20 transactions each.

Generate with the format:

for the transactions:
sample_01_<some-brief-description>.csv 
For the accounts' output:
sample_01_out.csv
